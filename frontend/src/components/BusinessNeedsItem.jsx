...
const BusinessNeedsItem = ({ id, title, media, priority, category }) => {
  ...
  const handleWishlist = (e) => {
    e.stopPropagation();
    if (toggleFavorite) toggleFavorite({ _id: id, name: title, media: [{ url: media }], price: priority });
  };
      ...
      <div className="dc__media">
        {media ? (
          isVideo
            ...
            : <img src={media} className="dc__img" alt={title} loading="lazy" />
        ) : (
        ...
        <div className="dc__hover-panel">
          <p className="dc__hover-name">{title}</p>
          <div className="dc__hover-row">
            {priority && <span className="dc__hover-price">Starting ₹{Number(priority).toLocaleString("en-IN")}</span>}
            <span className="dc__hover-btn">Explore →</span>
          </div>
        </div>
      </div>
      <div className="dc__body">
        <p className="dc__name">{title}</p>
        {priority && (
          <div className="dc__price-row">
            <span className="dc__price">Starting ₹{Number(priority).toLocaleString("en-IN")}</span>
          </div>
        )}
      </div>
    </div>
